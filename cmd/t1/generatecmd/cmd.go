...
import (
	...
	"github.com/senforsce/t1/cmd/t1/generatecmd/proxy"
	"github.com/senforsce/t1/cmd/t1/generatecmd/run"
	"github.com/senforsce/t1/cmd/t1/generatecmd/watcher"
...
func (cmd Generate) Run(ctx context.Context) (err error) {
	...
	if cmd.Args.IncludeVersion {
		opts = append(opts, generator.WithVersion(t1.Version()))
	}
	...
	// Check the version of the t1 module.
	if err := modcheck.Check(cmd.Args.Path); err != nil {
		cmd.Log.Warn("t1 version check: " + err.Error())
	}
