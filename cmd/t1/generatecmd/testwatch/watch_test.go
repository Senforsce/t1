...
func createTestProject(moduleRoot string) (dir string, err error) {
	dir, err = os.MkdirTemp("", "t1_watch_test_*")
	if err != nil {
...
func TestFileModificationsResultInSSE(t *testing.T) {
	...
	var eventsErr error
	go func() {
		eventsErr = readSSE(context.Background(), fmt.Sprintf("%s/_t1/reload/events", args.ProxyURL), events)
	}()
	...
	// Change file.
	t1File := filepath.Join(args.AppDir, "templates.t1")
	err = replaceInFile(t1File,
...
func Setup() (args TestArgs, teardown func(t *testing.T), err error) {
	...
	if err != nil {
		return args, teardown, fmt.Errorf("could not find local t1 go.mod file: %v", err)
	}
