...
func (h *FSEventHandler) HandleEvent(ctx context.Context, event fsnotify.Event) (goUpdated, textUpdated bool, err error) {
	...
	if !event.Has(fsnotify.Remove) && strings.HasSuffix(event.Name, "t1.go") {
		_, err = os.Stat(strings.TrimSuffix(event.Name, "t1.go") + ".t1")
	...
	if !event.Has(fsnotify.Remove) && strings.HasSuffix(event.Name, "t1.txt") {
		if h.DevMode {
...
func generateSourceMapVisualisation(ctx context.Context, templFileName, goFileName string, sourceMap *parser.SourceMap) error {
	...
	targetFileName := strings.TrimSuffix(templFileName, ".t1") + "_t1_sourcemap.html"
