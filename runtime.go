...
import (
	...
	"github.com/senforsce/t1/safehtml"
)
...
const componentHandlerErrorMessage = "t1: failed to render template"
...
const unknownTypeClassName = "--t1-css-class-unknown-type"
...
// use t1.Components skip rendering <style> elements for classes that are included in the global
// stylesheet. By default, the stylesheet path is /styles/t1.css
...
// the generator would have written non-go code to the _t1.txt file, which
...
func WriteWatchModeString(w *bytes.Buffer, lineNum int) error {
	...
	if !strings.HasSuffix(path, "_t1.go") {
		return errors.New("t1: WriteWatchModeString can only be called from _t1.go")
	}
	txtFilePath := strings.Replace(path, "_t1.go", "_t1.txt", 1)

	literals, err := getWatchedStrings(txtFilePath)
	if err != nil {
		return fmt.Errorf("t1: failed to cache strings: %w", err)
	}

	if lineNum > len(literals) {
		return errors.New("t1: failed to find line " + strconv.Itoa(lineNum) + " in " + txtFilePath)
	}
...
func getWatchedStrings(txtFilePath string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("t1: failed to stat %s: %w", txtFilePath, err)
	}
...
func cacheStrings(txtFilePath string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("t1: failed to open %s: %w", txtFilePath, err)
	}
	defer txtFile.Close()
	...
	if err != nil {
		return nil, fmt.Errorf("t1: failed to read %s: %w", txtFilePath, err)
	}
